...
export default function CastCrewSelector({ cast, onChange }: CastCrewSelectorProps) {
  ...
  const handleRoleChange = (index: number, role: 'actor' | 'producer') => {
    ...
    // Reset search state when role changes
  ...
  const { data, isFetching } = useQueryEvents(
    useQuery({
      ...
      enabled: !!searchQuery && searchQuery.length >= 2 && selectedMemberIndex !== null,
    }),
    {
      onSuccess: (data) => {
  ...
  useQueryEvents(
    useQuery({
      ...
      queryFn: async () => {
        if (!selectedPersonId || selectedMemberIndex === null || !cast[selectedMemberIndex]?.role) return null;
  ...
  const handleSelectPerson = (person: any) => {
    if (selectedMemberIndex !== null) {
      ...
      // Immediately update the name for better UX
      const newCast = [...cast];
      newCast[selectedMemberIndex] = {
        ...newCast[selectedMemberIndex],
        name: person.name,
          ...
          <div className="grid grid-cols-[1fr_auto] gap-3 items-start">
              ...
              <div className="relative" ref={searchRef}>
                ...
                {showCreateNew && selectedMemberIndex === index && (
                  <div className="absolute z-10 mt-1 w-full bg-cinema-800 border border-cinema-700 rounded-md p-3">
                    <p className="text-sm text-cinema-400 mb-2">No results found. Create new?</p>
                    <Button
                      type="button"
                      size="sm"
            ...
            <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
              {member.imageUrl ? (
                ...
                // Show image uploader only when creating a new entry
                member.name &&
                !member.id && (
                  <div className="w-24">
                    <Label className="sr-only">Profile Image</Label>
                    <FileUploader
                      onChange={(result: FileUploadResult) => handleImageChange(index, result)}
                      className="w-full"
