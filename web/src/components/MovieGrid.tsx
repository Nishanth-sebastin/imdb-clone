...
export default function MovieGrid({ movies, isUserMovies = false, isLoading = false }: MovieGridProps) {
  if (isLoading) {
    return (
      ...
      <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
        <div className="bg-cinema-800 p-8 rounded-lg max-w-md">
          ...
          <p className="text-cinema-400">
            {isUserMovies ? "You haven't added any movies yet." : 'There are no movies available at the moment.'}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
      {movies?.map((movie) =>
        isUserMovies ? <UserMovieCard key={movie.id} movie={movie} /> : <MovieCard key={movie.id} movie={movie} />
      )}
    </div>
  );
}
