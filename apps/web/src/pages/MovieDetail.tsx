...
const MovieDetail = () => {
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      const updatedMovie = await addReview(
        id,
        user.id,
        user.name,
        userRating,
  ...
  const canReview = user && movie.createdById !== user.id && 
                    !movie.reviews.some(review => review.userId === user.id);
      ...
      <main className="flex-1 page-container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="animate-slide-in" style={{ animationDelay: '0.1s' }}>
            <div className="relative overflow-hidden rounded-lg border border-cinema-800 animate-slide-in">
              <img 
                src={movie.posterUrl} 
                alt={movie.title}
              ...
              />
              {isUserMovie && (
                <div className="absolute bottom-0 left-0 right-0 bg-cinema-900/90 backdrop-blur p-4">
                  <Button 
          ...
          <div className="md:col-span-2 animate-slide-in" style={{ animationDelay: '0.2s' }}>
            <div className="bg-cinema-900 rounded-lg border border-cinema-800 p-6">
              ...
              <div className="flex flex-wrap gap-3 mt-4">
                ...
                {movie.createdByName && (
                  <div className="flex items-center space-x-1 text-sm bg-cinema-800 rounded-full px-3 py-1">
                    <User className="h-4 w-4 text-cinema-400" />
                    <span>Added by {movie.createdByName}</span>
                  </div>
                )}
              </div>
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {movie.actors.map((actor, index) => (
                    <span 
                      key={index}
                      className="text-sm bg-cinema-800 rounded-full px-3 py-1"
                    >
                      {actor}
                    </span>
                  ))}
                ...
                <div className="mt-8 border-t border-cinema-800 pt-6 animate-fade-in">
                  ...
                  <div className="mb-4">
                    <div className="flex items-center space-x-1 mb-2">
                      ...
                      {userRating > 0 && (
                        <span className="ml-2 text-gold font-medium">{userRating}/10</span>
                      )}
                    </div>
        ...
        <div className="mt-12 animate-slide-in" style={{ animationDelay: '0.3s' }}>
          ...
          {isUserMovie && (
            <div className="mt-8 text-center p-4">
              <p className="text-cinema-400 text-sm">
                This is your movie. You cannot review your own content.
              </p>
            </div>
          )}
        </div>
