...
const seedDatabase = async () => {
  ...
  try {
    ...
    const producers = await Producer.insertMany([
      {
        ...
        imageUrl:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRi86dnS_v65rt9-gxm0UNmW2-IOQKA4CBSug&s",
      },
      {
    ...
    const movies = await Movie.insertMany([
      {
        ...
        user_id: users[0]._id,
        cast: [
          { person: actors[0]._id, role: "actor" },
          { person: producers[0]._id, role: "producer" },
        ],
      },
      {
        ...
        cast: [
          { person: actors[0]._id, role: "actor" },
          { person: actors[1]._id, role: "actor" },
          { person: producers[1]._id, role: "producer" },
        ],
      },
    ]);
    ...
    for (const movie of movies) {
      const feedback = await MovieFeedback.find({ movie_id: movie._id });
      const ratings = feedback.map((f) => f.rating);
      const total = ratings.reduce((acc, curr) => acc + curr, 0);

      await Movie.findByIdAndUpdate(movie._id, {
        overall_ratings: ratings.length > 0 ? total / ratings.length : 0,
    ...
    await Promise.all([
      Actor.findByIdAndUpdate(actors[0]._id, {
